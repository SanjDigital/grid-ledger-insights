...
import { computeForensics } from "@/lib/forensic-engine-refactored";
import { useDecisionBasis, getMockDecisionBasis } from "@/hooks/useDecisionBasis";
...
/**
 * REFACTORED v2.0: Index.tsx now consumes pre-computed decision basis from API
 * 
 * Gap 6 Fix: Eliminates duplicate EAR and Trust Score computation.
 * All values now come from backend via useDecisionBasis hook.
 */
...
const IndexContent = () => {
  ...
  // This replaces local computation of EAR and Trust Score (Gap 6 fix)
  const { data: decisionBasis, loading: decisionBasisLoading, error: decisionBasisError } = useDecisionBasis(selectedMill.id);

  // Fallback to mock if API unavailable (for development)
  const effectiveDecisionBasis = decisionBasis || getMockDecisionBasis(selectedMill.id);

  // Physics variance override for demo
  const baseForensic = useMemo(() => ({
    ...forensicData,
    physicsVariance: varianceOverride ?? forensicData.physicsVariance,
  }), [varianceOverride]);

  // Compute supplementary forensics (per-event metrics, authority alignment)
  // This no longer computes EAR or Trust Score (those come from API)
  const computed = useMemo(() => 
    computeForensics(verifiedEvents, effectiveDecisionBasis, baseForensic), 
    [effectiveDecisionBasis, baseForensic]
  );

  // Build display forensic data
  // EAR and Trust Score now come from API via effectiveDecisionBasis
  const currentForensic = useMemo(() => ({
    ...baseForensic,
    currentSEC: computed.currentSEC,
    // API-PROVIDED VALUES (no longer computed locally):
    ear: computed.ear,
  ...
  if (decisionBasisError) {
    console.error("Failed to load decision basis:", decisionBasisError);
  }
    ...
    <div className="flex min-h-screen bg-background">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 overflow-auto p-5 space-y-5 scrollbar-terminal">
          {/* Debug: Show API loading state */}
          {decisionBasisLoading && (
            <div className="text-xs font-mono text-muted-foreground border border-border/50 p-2 rounded bg-secondary/30">
              ⏳ Fetching decision basis from API...
            </div>
          )}

          {decisionBasisError && (
            <div className="text-xs font-mono text-destructive border border-destructive/50 p-2 rounded bg-destructive/10">
              ⚠ API Error: {decisionBasisError.message}
            </div>
          )}
